...
iPho = find(strcmp(modelwt.rxnNames,'Phosphate exchange'));
iSul = find(strcmp(modelwt.rxnNames,'Sulfate exchange'));
...
for i = 1:nspecies
    model{i} = modelwt; 
    DB(i) = 1000;
    exID{i} = [iGr iGlu iFru iScr iEth iO2 iPho iSul]; 
end
...
VmaxPho = 5;            % maximum phosphate uptake rate (mmol/gDW/h)
KmPho = 1;              % phosphate uptake saturation constants (mmol/L)
VmaxSul = 5;            % maximum sulfate uptake rate (mmol/gDW/h)
KmSul = 1;              % sulfate uptake saturation constants (mmol/L)
param = [VmaxG; KmG; VmaxF; KmF; VmaxS; KmS; VmaxO; KmO; VmaxPho; KmPho; VmaxSul; KmSul];
...
Phoi = 1/136*1000;                              % mmol/L
Suli = 0.5/120*1000;                            % mmol/L

yz = [Xpbi, Xpri, Xchi, Gi, Fi, Si, Ei, Ogi, Oli, Phoi, Suli];
