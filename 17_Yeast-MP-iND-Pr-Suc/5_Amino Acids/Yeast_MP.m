...
Kla = 0.1;      % volumetric gas-liquid mass transfer coefficient (h-1)
...
KmG = 10;                % glucose uptake saturation constants (mmol/L)
VFmax = 10;              % maximum fructose uptake rate (mmol/gDW/h)
KmF = 10;                % fructose uptake saturation constants (mmol/L)
VSmax = 2;             % maximum sucrose uptake rate (mmol/gDW/h)
KmS = 0.1;                % sucrose uptake saturation constants (mmol/L)
...
for i = 3:2+species_num
    plot(time,pop_density_gL_mean(i-2,:),'LineWidth',2); 
...
fitMSE = mse(yinterp-pop_density_gL_mean(2,:));

%% Plot settings
species = {'DFBA','Exp Private 1% Sucrose','Exp Private 4% Sucrose', 'Interpolation'};
