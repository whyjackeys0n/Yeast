...
%% 3 Setup lexicographic optimization objectives
...
%% 4 Pass information to DFBAlab in INFO structure
INFO.nmodel = nmodel;  
INFO.DB = DB;         
INFO.exID = exID;
INFO.C = C; 

%% 5 Specify number of extracellular variables and pass to DFBAlab
ns = length(exID{1})*nmodel;
INFO.ns = ns;

%% 6 CSTR operation parameters and conditions
...
Sf  = 10;            % sucrose feed (mmol·L-1)
Ogf = Oc*P/R/Tr;    % oxygen feed in gas (mmol·L-1)

cond = [P, Tr, R, Oc, Kla, Oh];
feed = [Gf, Ff, Sf, Ogf];

%% 7 Set uptake parameters
VGmax = 3.5/180*1000;   % maximum glucose uptake rate (mmol/gDW/h)
KmG = 2.1;              % glucose uptake saturation constants (mmol/L)
...
%% 8 Pass conditions and parameters
INFO.D  = D;
INFO.Dg = Dg;
INFO.cond  = cond;
INFO.feed  = feed;
...
%% 10 Gurobi Objects construction parameters
