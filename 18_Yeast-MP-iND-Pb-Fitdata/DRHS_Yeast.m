...
% Induced synthesis rate
Betah = Betahm*G*180/1000/(Kbh+G*180/1000)/(1+G*180/1000/Kibhh);
pb_mu = max([pb_mus - Kp*(Alphah + Betah), 0]);
%pb_mu = pb_mus - Kp*(Alphah + Betah);

% Rate of reaction (sucrose-to-glucose reaction with glucose inhibition)
vdg = kdg*Inv*S*342/1000/(ks*(1+G*180/1000/kin)+S*342/1000)/180*1000;

% Extracellular balances
dXpb = -D*Xpb + pb_mu*Xpb;
dXpr = -D*Xpr + pr_mus*Xpr;
dXch = -D*Xch + ch_mus*Xch;
dG   = D*(Gf-G) + pb_vglc*Xpb + pr_vglc*Xpr + ch_vglc*Xch + vdg;
dF   = D*(Ff-F) + pb_vfru*Xpb + pr_vfru*Xpr + ch_vfru*Xch + vdg;
dS   = D*(Sf-S) + pb_vscr*Xpb + pr_vscr*Xpr + ch_vscr*Xch - vdg;
...
dInv = Alphah*Xpb + Betah*Xpb - Gamah*Inv;

fluxy = [fluxy, pb_mu,  pb_vglc, pb_vfru, pb_vscr, pb_veth, pb_voxy,...
                pr_mus, pr_vglc, pr_vfru, pr_vscr, pr_veth, pr_voxy,...
                ch_mus, ch_vglc, ch_vfru, ch_vscr, ch_veth, ch_voxy];
