...
figure(2)
subplot(1,3,1)
[errhigh,~] = max(pop_density_gL(1:7,:),[],1);
[errlow,~] = min(pop_density_gL(1:7,:),[],1);  
errorbar(time,pop_density_gL_mean(1,:),pop_density_gL_mean(1,:)-errlow,errhigh-pop_density_gL_mean(1,:),'-o','LineWidth',1);
legend('Private','FontSize',20)

subplot(1,3,2)
[errhigh,~] = max(pop_density_gL(8:14,:),[],1);
[errlow,~] = min(pop_density_gL(8:14,:),[],1);  
errorbar(time,pop_density_gL_mean(1,:),pop_density_gL_mean(1,:)-errlow,errhigh-pop_density_gL_mean(1,:),'-o','LineWidth',1);
legend('Cheat','FontSize',20)

subplot(1,3,3)
[errhigh,~] = max(pop_density_gL(15:21,:),[],1);
[errlow,~] = min(pop_density_gL(15:21,:),[],1);  
errorbar(time,pop_density_gL_mean(1,:),pop_density_gL_mean(1,:)-errlow,errhigh-pop_density_gL_mean(1,:),'-o','LineWidth',1);
legend('Public','FontSize',20)
