...
lb(2,2) = -VGmax * max([glc, 0]) / (KmG*1.056 + glc);
ub(2,2) = Inf;

lb(2,3) = -VFmax * max([fru, 0]) / (KmF*1.056 + fru);
ub(2,3) = Inf;
...
lb(2,6) = -VOmax * max([oxy, 0]) / (KmO + oxy);
ub(2,6) = Inf;
...
lb(3,2) = -VGmax * max([glc, 0]) / (KmG*1.056 + glc);
ub(3,2) = Inf;

lb(3,3) = -VFmax * max([fru, 0]) / (KmF*1.056 + fru);
ub(3,3) = Inf;
