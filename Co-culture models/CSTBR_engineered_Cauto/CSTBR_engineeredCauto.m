...
load Cauto_modelwt_engineered_6rxn.mat    
...
for i = 1:npoints
    ...
    DB(npoints+i) = 1000;
    exID{i}=[1026 960 854 972 969 986 658 966 1114];
...
condit = [klac,Hc,klac2,Hc2,klah,Hh,tr,eg,el,cgi,c2gi,hgi,gi,D,Dg];
...
while tint<tspan(2)
    ...
    INFO.INFtimes;
    [T,Y] = ode15s(@DRHS_engineeredCauto,tspan,Y0,options,INFO); 
    for i=1:length(T)
        [dy,fluxy] = DRHS_engineeredCauto(T(i),Y(i,:),INFO);
        flux = [flux;fluxy];
    end
    TF = [TF;T];
    YF = [YF;Y];
...
plot(T,ca_but(:,1))
xlabel('Time [h]')
ylabel('C.auto butyrate secretion rate [mmol/gDW/h]')
