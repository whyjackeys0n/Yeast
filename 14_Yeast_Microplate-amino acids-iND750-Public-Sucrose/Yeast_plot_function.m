...
species = {'Sim','Exp 1% Sucrose','Exp 4% Sucrose'};
...
fit_range = 24.9;
compare_range = 24.9;
[~,idx_T] = max(T>=fit_range,[],1);
f = @(c,x)c(1)./(1+exp(-c(2)*(x-c(3))));
c = lsqcurvefit(f,[20 0.4 25],T(1:idx_T),Y(1:idx_T,1));
[~,idx_time] = max(time>=compare_range,[],2);
fitMSE = mean((f(c,time(1:idx_time))-pop_density_gL_mean(1,1:idx_time)).^2);

%% Plot settings
plot(T(1:idx_T),f(c,T(1:idx_T)),'g--','LineWidth',3);
